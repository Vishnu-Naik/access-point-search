...
class OriginalBGOAS(Optimizer):
    ...
    [2] Mafarja, M., Aljarah, I., Faris, H., Hammouri, A. I., Al-Zoubi, A. M., & Mirjalili, S. (2019).
    Binary grasshopper optimisation algorithm approaches for feature selection problems.
    Expert Systems with Applications, 117, 267-286. https://doi.org/10.1016/j.eswa.2018.09.015
    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            for j in range(0, self.pop_size):
                dist = np.linalg.norm(self.pop[idx][self.ID_POS] - self.pop[j][self.ID_POS])
                ...
                s_ij = ran * self.s_function__(dist) * r_ij_vector
                S_i_total += s_ij
            x_new = c * np.random.normal() * S_i_total  # Eq. (2.7) in the paper
            # x_new = c * np.random.normal() * S_i_total + self.g_best[self.ID_POS]  # Eq. (2.7) in the paper
            x_new = 1 / ( 1 + np.exp(-x_new))
